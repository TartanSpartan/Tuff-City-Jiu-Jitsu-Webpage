...
# Recommended to basically reverse the structure of this file: build up BeltGrades, InstructorQualifications etc and then and only then the Users.
...
s = Syllabus.create(country:"canada")
s.save!
...
instructor_qualification_array = ["No Qualifications", "Assistant Instructor", "Instructor", "Club Instructor"] # A member can only have one of these at a time, and each successive one supplants the last
...
# Qualification.create!(level: "No Qualifications")
...
sbg= BeltGrade.create!({
    # user_id:1,
    belt_id: 1
    }) # Seumas, user_id:1, is a 1st Kyu so this id codifies his brown belt

dbg = BeltGrade.create!({
    # user_id:2,
    belt_id: 4
    }) # Likewise for David the 4th Kyu aka purple belt

siq = InstructorQualification.create!({
...
diq = InstructorQualification.create!({
...
20.times do
    ...
    if belt.id <= 2
        if random_instructors_qualifications_dice == true
            ...
            qualification_id = Qualification.third.id
        end
    end

    if belt.id >= 3 && belt.id <= 4 # 3 <= belt.id <= 4
        if random_instructors_qualifications_dice == true
    ...
    if belt.id >= 7 && belt.id <= 8
        # White and yellow belts aka belt.id = 8 and belt.id = 7 are not instructors
        qualification_id = Qualification.first.id
        achieved_at = nil
    end

    puts "Generating instructor qualifications", qualification_id

    if qualification_id != nil
        iq = InstructorQualification.create!({
            # user_id: user.id,
...
sbg.user_id = admin_user.id
sbg.save!

siq.user_id = admin_user.id
siq.save!
...
dbg.user_id = instructor_user.id
dbg.save!

diq.user_id = instructor_user.id
diq.save!
...
20.times do
    ...
    u = User.new( # Use User.new instead to inject other objects into this
    ...
    # Use instructorQualification.sample (including a false value)
    belts = Belt.all
    u.syllabus = s # Check if plural or singular here
    u.save!

end

for i in 3..22 do # Don't need to patch these attributes for Seumas or David
    u = User.find(id = i)
    bg = BeltGrade.find(id = i)
    bg.update(user_id: u.id)
    iq = InstructorQualification.find(id = i)
    iq.update(user_id: u.id, belt_grade_id: bg.id) 
    bg.update(belt_id: iq.belt_id) # In particular it's important for the belt_id to match between both objects; makes sense for instructor qualification to govern this based on boundaries of which one you need at which colour grade
end
